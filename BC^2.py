from sympy import Rational, sqrt, simplify, symbols, solve, expand
        ...
        for col in collinears:
            ...
            for k in k_pts:
                ...
                if pa and ab and ac and not bc and not pb:
                    ...
                    # → AK = AB×AC/AP
                    # ===========================
                    AK_val = simplify(ab_val * ac_val / pa_val)
                    # = 10×16/8 = 20
                    ...
                    # KC/KB = AC/AB (외각이등분선)
                    # KB = 5x, KC = 8x → BC = KC-KB = 3x
                    # KB×KC = 5x×8x = 40x²
                    # BC = 3x → x = BC/3
                    # KB×KC = 40(BC/3)² = 40BC²/9
                    ...
                    print(f"  AK={AK_val}, KP={KP_val}")
                    print(f"  방정식: 40BC²/9 = {KAKP}")
                    print(f"  → BC² = {simplify(KAKP * 9 / 40)}")
                    ...
                    if sol:
                        ...
                        pb_val  = simplify(
                            pa_val * bc_val / (ac_val + ab_val))

                        print(f"  BC²={bc2_val}, BC={bc_val}")

                        if add_derived(facts,
                            {"type": "length",
                             "line": b+c, "value": bc_val},
                            f"닮음방멱_{b+c}"):
                            changed = True
                        if add_derived(facts,
                            {"type": "length",
                             "line": p+b, "value": pb_val},
                            f"닮음방멱_{p+b}"):
                            changed = True
                        if add_derived(facts,
                            {"type": "length",
                             "line": p+c, "value": pb_val},
                            f"닮음방멱_{p+c}"):
                            changed = True
                        if add_derived(facts,
                            {"type": "length",
                             "line": a+k, "value": AK_val},
                            f"닮음_{a+k}=AB×AC/AP"):
                            changed = True
...
facts4 = {
    "entities": {
        "points": ["A", "B", "C", "P", "K"],
        "lines": ["AB", "AC", "BC", "PA", "PB", "PC", "PK", "AK"],
        "triangles": ["ABC", "ABP", "ACP", "AKB"],
        "circles": ["ω"]
    },
    "constraints": [
        {"type": "length",
